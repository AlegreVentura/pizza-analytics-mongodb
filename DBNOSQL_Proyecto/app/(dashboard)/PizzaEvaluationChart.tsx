...
interface PizzaEvaluacion {
  ...
  estrategia: 'Mantener' | 'Promocionar' | 'Ajustar precio' | 'Descontinuar' | 'Estrella';
}
...
export default function PizzaEvaluationChart() {
  ...
  if (data.length === 0) {
    return <Typography>Sin datos disponibles</Typography>;
  }
  ...
  const series = [
    {
      ...
      label: 'Estrella',
      data: data
        .filter((item) => item.estrategia === 'Estrella')
        .map((item) => ({
          x: item.total_unidades,
          y: item.precio_promedio,
          id: item.pizza_name,
        })),
      color: coloresEstrategia['Estrella'],
