  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Promedio semanal como referencia — días por debajo del promedio son candidatos a promoción
        const avgVentas = formattedData.reduce((s: number, d: HeatmapPoint) => s + d.ventas, 0) / formattedData.length;

        const dataWithExtras: EnrichedPoint[] = formattedData.map(({ x, y, ventas }: HeatmapPoint) => {
          const gap = Math.max(avgVentas - ventas, 0); // 0 si ya supera el promedio
          const recomendacion = Math.round((gap / avgVentas) * 100);
          const ganancia = Math.round(gap * 16); // pizzas faltantes × precio promedio ~$16
          return { x, y, value: ganancia, ventas, recomendacion };
        });

        const maxGanancia = Math.max(...dataWithExtras.map((d) => d.value), 1);
        const colorMax = Math.ceil(maxGanancia / 100) * 100; // redondear al siguiente 100
        ...
        setChartOptions({
          ...
          colorAxis: {
            min: 0,
            max: colorMax,
            tickInterval: Math.ceil(colorMax / 3 / 100) * 100,
            stops: [
